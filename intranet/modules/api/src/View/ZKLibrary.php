...
class ZKLibrary
{
    ...
    public function getAttendanceDate($date)
    {
        $date1 = $date . ' 00:00:00';
        $date2 = $date . ' 24:00:00';
    ...
    public function getAttendanceUser($date)
    {
        $times = $this->getAttendanceDate($date);
        // $users = $this->getUser();
        $dataResult = [];
            ...
            //         ];
            //     }
            // } bỏ key name nên comment lại vì không cần mảng này
        if (count($times)) {
            foreach ($times as $time) {
                if (!empty($time[3])) {
                    $dataResult[$time[1]][] = $time[3];
                }
            }
        }
