                    ...
                    <div class="form-horizontal col-md-12">
                        <form id="frm_create_education" method="post"
                            @if($isScopeHrOrCompany)
                                action="{{ URL::route('education::education.request.hr.update', ['id' => $education->id]) }}" autocomplete="off"
                            @else
                                action="{{ URL::route('education::education.request.update', ['id' => $education->id]) }}" autocomplete="off"
                            @endif>
                            @include('education::education-request._form')
                            <div class="row btn-container">
                                <div class="col-md-12 text-align-center margin-bottom-30" >
                                    @if($isScopeHrOrCompany || $education->status == $statusRequest)
                                        <button class="btn btn-primary btn-create" type="submit">
                                            {{ trans('education::view.Education.Update') }}
                                            <i class="fa fa-spin fa-refresh hidden"></i>
                                        </button>
                                    @endif
                                    <a class="btn btn-danger" href="{{ $isScopeHrOrCompany ? route('education::education.request.hr.list') : route('education::education.request.list') }}">
...
@section('script')
    ...
    <script>
        ...
        var assignId = "{{ isset($education->assigned->id) ? $education->assigned->id : old('assign_id') }}";
    </script>
