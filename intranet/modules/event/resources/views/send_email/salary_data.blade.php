...
$totalCol++;
$arrFloatIdxs = [$rowIndexs['ot'], $rowIndexs['cong_thu_viec'], $rowIndexs['cong_chinh_thuc']];
?>
<div class="box box-info">
    ...
    <div class="box-body">
        <div class="row">
            <div class="col-md-6">
                <h4 class="box-title">{{ $salaryFile->filename }}</h4>
                <p class="text-yellow">({{ trans('event::view.Data is only temporarily stored') }})</p>
            </div>
            <div class="col-md-3">
                @include('event::salary.links')
            </div>
            <div class="col-md-3">
                <div class="group-button text-right">
                    {!! Form::open(['method' => 'delete', 'route' => 'event::send.email.employees.delete_temp.salary', 'class' => 'form-inline no-validate', 'id' => 'delete_salary_form']) !!}
            ...
            <tbody>
                ...
                <tr data-email="{{ $email }}">
                    ...
                    <td class="white-space-nowrap">
                        @if (isset($row[$i]))
                        {{ in_array($i, $arrFloatIdxs) ? $row[$i] : ViewEvent::formatMoney($row[$i]) }}
                        @endif
                    </td>
                    @endfor
                </tr>
...
<script>
    var stopSending = 0;
    var sendMailUrl = '{{ route("event::send.email.employees.send_mail.salary") }}';
    var delaySendMail = {{ ViewEvent::DELAY_SEND_MAIL }};
    var detailMailSentLink = '';
    @if ($salaryFile)
        detailMailSentLink = '{{ route("event::send.email.employees.salary.mail_detail", ["id" => $salaryFile->id]) }}';
    @endif
</script>
