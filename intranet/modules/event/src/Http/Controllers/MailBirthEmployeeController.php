...
class MailBirthEmployeeController extends Controller
{
    /**
     * construct more
     */
    ...
    public function index()
    {
        ...
        Breadcrumb::add('Manage send mail birthday employee');
        return view('event::employee_noti.birthday_manage', [
            'userCurrent' => Permission::getInstance()->getEmployee(),
            'contentEmail' => CoreConfigData::getValueDb('event.mail.bitrhday.employee.content'),
            'subjectEmail' => CoreConfigData::getValueDb('event.mail.bitrhday.employee.subject'),
        ]);
    }
    ...
    public function save()
    {
        ...
        $nameInputContent = 'event.mail.bitrhday.employee.content';
        $nameInputSubject = 'event.mail.bitrhday.employee.subject';
    ...
    protected function preview()
    {
        ...
        $htmlView = view('event::employee_noti.mail.birth',[
            'data' => [
                'employee' => $employee
            ]
        ])->render();
        $dataSubject = MailEmployee::patternsNotiBirtday($employee, [
    ...
    protected function sendDemo()
    {
        ...
        $response = [];
        try {
            Mail::send('event::employee_noti.mail.birth', [
                'data' => [
                    'employee' => $employee
                ]
            ...
            $response['message'] = Lang::get('event::message.Canot send mail');
            $response['error'] = 1;
            Log::info($ex);
            return $response;
        }
