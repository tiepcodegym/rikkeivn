...
namespace Rikkei\HomeMessage\View;
...
class FileUploader
{
    ...
    const CONST_GB = 1073741824; //1073741824 bytes = 1GB
    const CONST_MB = 1048576;  //1048576 bytes = 1Mb
    const CONST_KB = 1024; //1048576 bytes = 1KB

    protected $dirUploadFile; //Dir storage file
    protected $filePath;//Path file
    protected $storage; //Object storage
     ...
     */
    public function __construct($name, $folderPath, $options = null)
    {
        $this->dirUploadFile = $folderPath;
        $this->filePath = "/storage$folderPath";
        $this->storage = Storage::disk('public');
        if (!$this->storage->exists($folderPath)) {
            $mkDir = $this->storage->makeDirectory($folderPath);
            if (!$mkDir) {
                throw new \Exception("Can't not create folder $folderPath");
            }
        }
        ...
        if ($this->field['input']) {
            ...
            if ($validate === true) {
                ...
                for ($i = 0; $i < count($this->field['input']['name']); $i++) {
                    ...
                    $metas['replaced'] = $this->storage->exists($this->dirUploadFile . $metas['name']);
                    $metas['date'] = date('r');
                    ksort($metas);
                ...
                if (!$data['hasWarnings']) {
                    foreach ($data['files'] as $key => $file) {
                        $file['name'] = str_random(5) . '_' . time() . '.' . $file['extension'];
                        $this->storage->put($this->dirUploadFile . $file['name'], file_get_contents($file['tmp_name']));
                        unset($data['files'][$key]['tmp_name']);
                        $data['files'][$key]['uploaded'] = true;
                        $data['files'][$key]['file'] = $this->filePath . $file['name'];
    ...
    {
        if ($bytes >= self::CONST_GB) {
            $bytes = number_format($bytes / self::CONST_GB, 2) . ' GB';
        } elseif ($bytes >= self::CONST_MB) {
            $bytes = number_format($bytes / self::CONST_MB, 2) . ' MB';
        } elseif ($bytes > 0) {
            $bytes = number_format($bytes / self::CONST_KB, 2) . ' KB';
        } else {
            $bytes = '0 bytes';
        }
    ...
    private function generateFilename($conf, $file, $skip_replace_check = false)
    {
        ...
        if (!$this->options['replace'] && !$skip_replace_check) {
            $title = $file['title'];
            $i = 1;
            while ($this->storage->exists($this->dirUploadFile . $string)) {
                $file['title'] = $title . " ({$i})";
    ...
    public function uploadDynamic($nameFolder)
    {
        return $this->uploadFileDynamic($nameFolder);
    }

    /**
     * can change name folder when save
     * uploadFiles method
     * Process and upload the files
     *
     * @private
     * @return {null, Array}
     */
    private function uploadFileDynamic($nameFolder)
    {
        $data = array(
            "hasWarnings" => false,
            "isSuccess" => false,
