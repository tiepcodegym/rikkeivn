...
@section('title-manage')
    {{ trans('manage_time::view.Manage supplement') }} 
@endsection
...
<?php
    ...
    $urlShowPopup = route('manage_time::profile.supplement.view-popup');
    $allowCreateEditOther = SupplementPermission::allowCreateEditOther();
    $colspan = 10;
        ...
        <div class="box-header">
                    ...
                    <div class="input-box">
                        <select name="team_all" id="select-team-member"
                            ...
                            @if ($teamIdsAvailable === true)
                                <option value="{{ URL::route('manage_time::timekeeping.manage.supplement') }}"<?php
                            ...
                            @if ($teamIdsAvailable === true || (count($teamsOptionAll) && $teamTreeAvailable))
                                @foreach($teamsOptionAll as $option)
                                    @if ($teamIdsAvailable === true || in_array($option['value'], $teamTreeAvailable))
                                        <option value="{{ URL::route('manage_time::timekeeping.manage.supplement', ['team' => $option['value']]) }}"<?php
            ...
            <div class="pull-right">
                <div class="filter-action">
                    @if ($allowCreateEditOther)
                        <a class="btn btn-success managetime-margin-bottom-5" href="{{ route('manage_time::profile.supplement.admin-register') }}" target="_blank">
                                ...
                                ?>
                                    <tr>
                                        ...
                                        @else
                                            <td class="{{ $tdClass }}"></td>
                                        @endif
                                        <td class="{{ $tdClass }}">{{ $item->number_days_supplement }}</td>
                                        ...
                                        @if ($allowCreateEditOther)
                                            <td>
                                                ...
                                                @else
                                                    <a class="btn btn-success managetime-margin-bottom-5" title="{{ trans('team::view.Edit') }}" href="{{ route('manage_time::profile.supplement.edit', ['id' => $item->register_id]) }}" >
                                                        <i class="fa fa-edit"></i>
                                                    </a>
                                                    <button class="btn btn-danger button-delete managetime-margin-bottom-5" value="{{ $item->register_id }}" data-status="{{ $item->status }}" title="{{ trans('team::view.Delete') }}" data-toggle="modal">
...
@section('script-manage')
    ...
    <script type="text/javascript">
        ...
        var urlDelete = '{{ route('manage_time::profile.supplement.delete') }}';
        ...
        $(window).scroll(function() {
            ...
            if (scrollTop > fixTop) {
                ...
                var colActionUser = $('#managetime_table_primary .col-action-user').width();
                $('#managetime_table_fixed .col-action-user').width(colActionUser);
            }
        });
    </script>
@endsection
