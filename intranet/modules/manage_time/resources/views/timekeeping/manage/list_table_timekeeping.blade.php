...
@section('content')
    <div class="box box-primary">
        ...
        <!-- Modal get data timekeeping  wfh-->
        <div id="modal_get_data_timekeeping-aggregate-wfh" class="modal fade" role="dialog" data-backdrop="static">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title" style="font-size: 23px;">
                            Cập nhật bảng chấm công tổng hợp.
                        </h4>
                    </div>
                    <form method="POST" action="{{ route('manage_time::timekeeping.get-data-aggregate-module-wfh') }}">
                        {{ csrf_field() }}
                        <input type="hidden" name="timekeeping_table_id" class="form-control timekeeping_table_id_data" />
                        <div class="modal-body">
                            <div class="row">
                                <div class="col-sm-12 managetime-form-group">
                                    <label class="control-label">Chọn nhân viên để tổng hợp công <sup>(1)</sup></label>
                                    <div class="input-box">
                                        <select name="empids[]" class="form-control select-search-employee" data-remote-url="{{ URL::route('manage_time::profile.comelate.ajax-search-employee') }}" multiple>
                                        </select>
                                    </div>
                            ...
                            <div class="row">
                                <div class="col-sm-12 managetime-form-group">
                                    <label id="error-aggregate" class="managetime-error" for="start_date"></label>
                                </div>
                            </div>
                            <hr>
                            <div class="row">
                                <div class="col-xs-12">
                                    <b>{{ trans('manage_time::view.Attention:') }}</b>
                                    <ul>
                                        <li>(1): Để trống sẽ tổng hợp công cho toàn bộ nhân viên</li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-default pull-left" data-dismiss="modal">{{ trans('manage_time::view.Close') }}</button>
                            <button type="submit" class="btn-add" data-loading-text="<i class='fa fa-spin fa-refresh'></i> {{ trans('manage_time::view.Yes') }}"> {{ trans('manage_time::view.Yes') }}</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <script type="text/javascript">
        ...
        jQuery(document).ready(function ($) {
            ...
            $('.btn-update-timekeeping-aggregate').click(function() {
                var data = $(this).closest('tr');
                $('#modal_get_data_timekeeping-aggregate-wfh .timekeeping_table_id_data').val(data.attr('timekeeping-table-id'));
                $('#modal_get_data_timekeeping-aggregate-wfh').modal('show');
