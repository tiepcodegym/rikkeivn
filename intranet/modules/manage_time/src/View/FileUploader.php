            ...
            if ($validate === true) {
                ...
                if (!$data['hasWarnings']) {
                    foreach ($data['files'] as $key => $file) {
                        ...
                        Storage::put('public/manage-time/'. $file['name'],file_get_contents($file['tmp_name']));
                        unset($data['files'][$key]['tmp_name']);
                        $data['files'][$key]['uploaded'] = true;
                        $data['files'][$key]['file'] = 'storage/manage-time/'.$file['name'];
                        $this->options['files'][] = $data['files'][$key];
                        @chmod(storage_path('app/' . 'public' . '/' . 'manage-time' . '/' . $file['name']), 0777);
                    }
                }
            } else {
                $data['isSuccess'] = false;
        ...
        if ($this->field['input']) {
            ...
            if ($validate === true) {
                ...
                if (!$data['hasWarnings']) {
                    foreach ($data['files'] as $key => $file) {
                        $file['name'] = str_random(5) . '_' . time() . '.' .$file['extension'];
                        Storage::put('public/' . $nameFolder . '/' . $file['name'], file_get_contents($file['tmp_name']));
                        unset($data['files'][$key]['tmp_name']);
                        $data['files'][$key]['uploaded'] = true;
                        $data['files'][$key]['file'] = 'storage/' . $nameFolder . '/' . $file['name'];
                        $this->options['files'][] = $data['files'][$key];
                        @chmod(storage_path('app/' . 'public' . '/' . $nameFolder . '/' . $file['name']), 0777);
                    }
                }
            } else {
                $data['isSuccess'] = false;
