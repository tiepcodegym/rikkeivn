...
$teamTreeHtml = TeamList::getTreeHtml(Form::getData('id'), $teamIds);
            ...
            <div class="team-position-wrapper hight-same">
                <div class="box box-info">
                    ...
                    <div class="row team-list-action box-body">
                        <form id="form-post-edit" method="post"
                              action="{{ route('notify::admin.notify.update', $notifyMobile->id) }}"
                            ...
                            <div class="form-group">
                                <div class="col-md-8 col-md-offset-2">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="">
                                            <input name="title" class="form-control input-field" type="text"
                                                   value="{{$notifyMobile->title }}" id="title"/>
                                        </div>
                                    ...
                                    <textarea id="content" name="content" class="form-control"
                                              rows="5">{{ $notifyMobile->content }}</textarea>
                                        </div>
                                        ...
                                        <div class="">
                                            ...
                                            <div class="form-check">
                                                <input class="form-check-input" type="radio" name="date_type" value="2"
                                                       {{ old('date_type') == 2 || isset($notifyMobile->available_at) ? 'checked' : null }}
                                                       id="date_type2">
                                                <label class="form-check-label" for="date_type2">{{ trans('notify::view.choose_date') }}</label>
                                            </div>
                                            <div class='input-group date'
                                                 style="{{ old('date_type') == 2 || isset($notifyMobile->available_at) ? null  : 'display: none' }}"
                                                 id='datepicker_start_at'>
                                                <input type='text' class="form-control"
                                                       name="available_at"
                                                       {{ old('date_type') == 2 || isset($notifyMobile->available_at) ? null : 'disabled' }}
                                                       value="{{ old('available_at', isset($notifyMobile->available_at) ? $notifyMobile->available_at : \Carbon\Carbon::now()->addMinute(2)->format('d-m-Y H:i')) }}"
                                                       id="available_at"/>
