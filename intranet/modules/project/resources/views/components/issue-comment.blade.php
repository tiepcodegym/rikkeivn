...
@if (isset($issueInfo))
    <div class="box box-primary box-solid">
        ...
        <div class="box-body">
            <div class="row">
                <form id="form-issue-comment" method="post" action="{{ route('project::issue.save.comment') }}"  autocomplete="off" data-callback-success="commentSuccess" novalidate="novalidate" enctype="multipart/form-data">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <input type="hidden" name="issue_id" value="{{ $issueInfo->id }}">
                    <input type="hidden" name="project_id" value="{{ $projectId }}">
                    <input type="hidden" name="comment_id" value="">
                    <div class="col-md-10 form-group" id="b-dropzone-wrapper">
                        <textarea name="content" class="form-control text-resize-y" rows="3" id="comment" value=""></textarea>
                        <span class="text-esc hidden" style="font-size: 11px; margin-left: 5px;">Nhấn Esc để hủy edit</span>

                        <div>
                            <label for="attach_comment" style="color:blue;"><i class="fa fa-upload" aria-hidden="true"></i>Attach a file</label>
                            <input type="file" id="attach_comment" name="attach_comment[]" style="display: none" multiple />
                        </div>
                    </div>
...
<script>
    (function ($) {
        ...
        $('#attach_comment').change(function(e) {
        ...
        $('#comment').mentionsInput({
            ...
            templates:{
                // mentionItemHighlight: _.template('<strong><span><%= name %></span><input type="hidden" name="emp_mention[]" value="<%= id %>" /></strong>')
