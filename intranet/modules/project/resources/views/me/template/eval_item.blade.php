...
<tr data-eval="{{$item->id}}" data-project="{{$project->id}}" data-time="{{$item->eval_time}}" data-edit="{{ $canChangePoint }}">
    ...
    <td class="minw-100"><a href="{{ route('project::point.edit', ['id' => $project->id]) }}" target="_blank">{{ $project->name }}</a></td>
                ...
                <div class="input_select">
                    @if (!$canChangePoint)
                        @if ($attr->group == MeAttribute::GR_NORMAL)
                            ...
                            @else
                                <input type="number" class="form-control _me_attr_point"
                                       data-value="{{ $attrPoint }}" disabled value="{{ $attrPoint }}"
                                       autocomplete="off" data-attr="{{$attr->id}}" data-weight="{{$attr->weight}}"
                                       min="{{$range_min}}" max="{{$range_max}}" step="{{$range_step}}" style="padding: 6px;">
                            @endif
                        @else
                            <select disabled class="form-control select-search _me_attr_point minw-110"
                                    data-integer="true" data-value="{{ $attrPoint }}"
                                    data-attr="{{$attr->id}}" data-weight="{{$attr->weight}}" style="padding: 6px;">
                                {!! $item->optionsPoint($attrPoint, $attr->has_na) !!}
                            </select>
                        @endif
                    @else
                        @if ($range_step > 1)
                            <select class="form-control select-search _me_attr_point"
                                    data-integer="true" data-value="{{ $attrPoint }}"
                                    data-attr="{{$attr->id}}" data-weight="{{$attr->weight}}" style="padding: 6px;">
                        ...
                        @elseif ($attr->group != MeAttribute::GR_NORMAL)
                            <select class="form-control select-search _me_attr_point minw-110"
                                    ...
                                    data-weight="{{$attr->weight}}" min="{{$range_min}}"
                                    max="{{$range_max}}" style="padding: 6px;">
                                {!! $item->optionsPoint($attrPoint, $attr->has_na) !!}
                            </select>
                        ...
                        @else
                            <input type="number" data-value="{{ $attrPoint }}"
                                    ...
                                    @if(filter_var($range_step, FILTER_VALIDATE_INT))
                                        ...
                                        data-integer="true"
                                    @endif
                                    {{ (!$attr->can_fill && $existsMonth) ? 'disabled' : '' }} value="{{$attrPoint}}"
                                    autocomplete="off" data-attr="{{$attr->id}}" data-weight="{{$attr->weight}}"
                                    min="{{$range_min}}" max="{{$range_max}}" step="{{$range_step}}" style="padding: 6px;">
                        @endif
                    @endif
                </div>
                @include('project::me.template.comments', ['user' => $currUser, 'item_id' => $item->id, 'attr_id' => $attr->id, 'project_id' => $project->id])
            </td>
        @endforeach
    @endif
    <td class="_pf_person_avg auto_fill _none"></td>
    <td class="_project_point auto_fill">{{$projectPoint}}</td>
    <td class="_project_type auto_fill">{{$item->getFactorProjectType()}}</td>
    ...
    <td class="auto_fill">{{$item->memberDaysInMonth($item->work_dates)}}</td>
    <td class="_contribute_val _break_word auto_fill">
        {{$item->contribute_label}}
    </td>
    <td class="note_group {{$item->hasComments()}}">
        @include('project::me.template.comments', ['user' => $currUser, 'item_id' => $item->id, 'attr_id' => null, 'project_id' => $project->id, 'comment_type' => MeComment::TYPE_NOTE])
    </td>
