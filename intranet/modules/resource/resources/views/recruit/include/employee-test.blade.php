...
$listResults[getOptions::RESULT_DEFAULT] = trans('resource::view.Testing');
?>
            ...
            <table class="table table-striped table-bordered table-hover table-grid-data">
                <thead>
                    <tr class="bg-light-blue">
                        ...
                        <th>{{ trans('resource::view.Test schedule') }}</th>
                ...
                <tbody>
                    <tr>
                        ...
                        <td></td>
                        <td>
                            <input type="text" class='form-control filter-grid' name="filter[{{ $cddTbl }}.test_plan]" value="{{ FormView::getFilterData($cddTbl.'.test_plan') }}" placeholder="{{ trans('team::view.Search') }}..." />
                        </td>
                        <td>
                            <?php $testResultFilter = FormView::getFilterData('excerpt', 'result'); ?>
                            <select style="min-width: 80px;" name="filter[excerpt][result]" class="form-control select-grid filter-grid select-search">
                                ...
                                <option value="{{ $value }}" {{ is_numeric($testResultFilter) && $testResultFilter == $value ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        </td>
                        <td></td>
