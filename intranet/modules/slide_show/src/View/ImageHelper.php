...
class ImageHelper
{
    ...
    public function __construct($image = null) {
        $this->cacheFolderShort = Config::get('general.upload_folder') .'/'. FileModel::PATH_DEFAULT_IMAGE_CACHE . '/';
    ...
    private function processImageInfo($image, $pathSave = 0)
    {
        ...
        switch ($pathSave) {
            case 0: // use path default
                $fullImageCachePath = $this->cacheFolder;;
                break;
            case 1: // use path type 2, only render
                $path = Config::get('general.upload_folder');
                $shortPath = preg_replace('/^' . trim($path, '/') . '/', '', $shortPath);
                $shortPath = FileModel::PATH_DEFAULT_IMAGE_CACHE . $shortPath;
                $fullImageCachePath = public_path($path. '/' . $shortPath);
                break;
        }
        //create folder
        if (! File::exists($fullImageCachePath)) {
            try {
                if (! File::makeDirectory($fullImageCachePath, 0775, true, true)) {
                    throw new Exception(Lang::get('frontend_error.Cannot create folder image cache'));
                }
            } catch (Exception $ex) {
                throw $ex;
            }
        ...
        //$imageInfo['capacity'] = filesize($fullPath) / (1000 * 1000); //MB
        return $imageInfo;
    }
        ...
        if ($file->isValid()) {
            ...
            if ($allowType) {
                ...
                if (! in_array($extension, $allowType)) {
                    throw new Exception(Lang::get('frontend_error.File type dont allow upload'), CoreModel::ERROR_CODE_EXCEPTION);
                }
            }
            if ($capacity) {
                if ($capacity < $file->getClientSize() / pow(Config::get('image.capacity_unit'),2)) {
                    throw new Exception(
                        ...
                        CoreModel::ERROR_CODE_EXCEPTION
                    );
                }
            }
            if ($rename) {
