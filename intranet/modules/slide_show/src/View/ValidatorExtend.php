...
class ValidatorExtend
{
    ...
    {
        $class = get_class();
        Validator::extend('repeat_slide', "{$class}@validateRepeatSlide");
        Validator::extend('type_slide', "{$class}@validateTypeSlide");
        Validator::extend('unique_hour_start', "{$class}@validateUniqueHourStart");
        Validator::extend('unique_repeat', "{$class}@validateUniqueRepeat");
        Validator::extend('option_slide', "{$class}@validateOptionSlide");
        Validator::extend('type_lang', "{$class}@validateTypeLang");
        Validator::extend('slide_quotation', "{$class}@validateSlideQuotation");
        Validator::extend('slide_birthday', "{$class}@validateSlideBirthday");
    }
    ...
    /**
     * validate option slider
     * @param   $attribute  
     * @param   $value      
     * @param   $parameters 
     * @return  boolean          
     */
    public function validateOptionSlide($attribute, $value, $parameters)
    {
        $allOptionSlide = Slide::getAllOptionSlide();
        if(!in_array($value, array_keys($allOptionSlide))) {
            return false;
        }
        return true;
    }
    ...
    /**
     * validate type lang slide
     * @param   $attribute  
     * @param   $value      
     * @param   $parameters 
     * @return  boolean          
     */
    public function validateTypeLang($attribute, $value, $parameters)
    {
        $allLangSlide = Slide::getAllLanguageSlide();
        if(!in_array($value, array_keys($allLangSlide))) {
            return false;
        }
        return true;
    }
     ...
     * @return boolean
     */
    public static function validateSlideQuotation($attribute, $value, $parameters)
    {
        if (!$value || !count($value)) {
            return false;
        }
        foreach ($value as $key => $items) {
            if (!in_array($key, ['new', 'old'])) {
                continue;
            }
     ...
     * @return boolean
     */
    public static function validateSlideBirthday($attribute, $value, $parameters)
    {
        if (!$value || !count($value)) {
            return false;
        }
