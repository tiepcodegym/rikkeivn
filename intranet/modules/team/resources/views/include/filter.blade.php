        ...
        @foreach ($buttons as $button)
            @if (isset($button['type']) && $button['type'] == 'link')
                <a href="<?php if (isset($button['url']) && $button['url']): ?>{{ $button['url'] }}<?php endif; ?>"
                    ...
                    <?php if (isset($button['tooltip']) && $button['tooltip']): ?> data-toggle="tooltip" data-html="true" title="{!! $button['tooltip'] !!}"<?php endif; ?>
            ...
            @else
                <button type="button" class="add-general-task btn btn-primary{{ isset($button['class']) ? ' ' . $button['class'] : '' }}"
                    ...
                    <?php if (isset($button['url']) && $button['url']): ?> data-url="{{ $button['url'] }}"<?php endif; ?>
                    <?php if (isset($button['tooltip']) && $button['tooltip']): ?> data-toggle="tooltip" data-html="true" title="{!! $button['tooltip'] !!}"<?php endif; ?>
                >{!! trans($domainTrans . '::view.' . $button['label']) !!}
                    <?php if (isset($button['square']) && $button['square']): ?><i class="fa fa-check-square-o hidden"></i><?php endif; ?>
                    <?php if (isset($button['icon_refresh']) && $button['icon_refresh']): ?> <i class="fa fa-spin fa-refresh hidden submit-ajax-refresh-btn"></i><?php endif; ?></button>
            @endif
        @endforeach
    @endif
