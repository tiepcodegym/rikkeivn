...
@section('title', trans('team::view.Upload member'))
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group form-label-left row">
                            <label class="col-md-2 control-label" for="excel_file">{{ trans('team::view.Member List (CSV or Excel format)') }}</label>
                            <div class="col-md-6">
...
$teams = UploadMember::getTeamplateInFlagTeamCode("title");
$positions = UploadMember::getTeamplateInFlagPositionCode();
?>
<div class="box box-info">
    <div class="box-body">
        <h4>
            <a href="{{ URL::to('help/template/Rikkeisoft-XuatKhauHoSo-Template.xls') }}">{{ trans('team::view.Download a template') }}</a>
        </h4>
        <div>
            <p><strong>{{ trans('team::view.Date format') }}</strong>: {{ trans('team::view.d/m/Y | d-m-Y') }}</p>
        </div>
        <div>
            <p><strong>{{ trans('team::view.Team code') }}</strong></p>
            <div class="row">
                @foreach ($teams as $teamCode => $teamName)
                    <div class="col-md-4">
                        <p>{{ $teamCode . ': ' . $teamName }}</p>
                    </div>
                @endforeach
            </div>
        </div>
        <div>
            <p><strong>{{ trans('team::view.Position code') }}</strong></p>
            <div class="row">
                @foreach ($positions as $teamCode => $teamName)
                    <div class="col-md-4">
                        <p>{{ $teamCode . ': ' . $teamName }}</p>
                    </div>
                @endforeach
