            ...
            if ($validate === true) {
                ...
                if (!$data['hasWarnings']) {
                    foreach($data['files'] as $key => $file) {
                        ...
                        Storage::put('public/ticket/'. $file['name'],file_get_contents($file['tmp_name']));
                        unset($data['files'][$key]['tmp_name']);
                        $data['files'][$key]['uploaded'] = true;
                        $data['files'][$key]['file'] = 'storage/ticket/'.$file['name'];
                        $this->options['files'][] = $data['files'][$key];
                        @chmod(storage_path('app/' . 'public' . '/' . 'ticket' . '/' . $file['name']), 0777);
                    }
                }
            } else {
                $data['isSuccess'] = false;
