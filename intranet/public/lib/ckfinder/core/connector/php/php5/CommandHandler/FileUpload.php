...
require_once dirname(CKFINDER_CONNECTOR_CONFIG_FILE_PATH) . '/../../../bootstrap/autoload.php';
...
class CKFinder_Connector_CommandHandler_FileUpload extends CKFinder_Connector_CommandHandler_CommandHandlerBase
{
    ...
    public function sendResponse()
    {
        ...
        $sFileName = $this->fileSplitAndEncodeName($sFileName);
    ...
    protected function fileSplitAndEncodeName ($fileFullName)
    {
        $pos = strrpos($fileFullName, '.');
        if ($pos === false) {
            return Str::ascii($fileFullName);
        }
        return Str::ascii(substr($fileFullName, 0, $pos)) . substr($fileFullName, $pos);
    }
}
